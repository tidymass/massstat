#' Conflicts between the massstat and other packages
#'
#' This function lists all the conflicts between packages in the massstat
...
massstat_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", massstat_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "massstat_conflicts")
}

massstat_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "massstat_conflicts()"
  )
...
#' @export
print.massstat_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(massstat_conflict_message(x))
}
